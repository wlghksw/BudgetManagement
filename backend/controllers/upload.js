...
const categorizeTransaction = async (description, userId, transactionType) => {
  ...
  if (transactionType === 'expense') {
    const rules = {
      // 식비: 가장 많은 키워드로 우선순위 높임
      '식비': [
        // 카페/커피 (가장 먼저 확인)
        '카페', '커피', '커피숍', '수소수카페', '컴포즈커피', '빽다방', '이디야', '투썸', 
        '할리스', '탐앤탐스', '카페베네', '엔젤리너스', '해피카페', '스타벅스',
        // 식당/음식점 (두 번째로 확인) - 대학교 식당도 포함
        '식당', '교수회관식당', '학생식당', '구내식당', '카페테리아', '레스토랑', 
        '맛집', '음식', '식사', '이대조뼈다귀', '이가네', '삼겹살', '고기', 'BBQ',
        '백석대학교 교수회관식당', '대학교 식당', '학교식당',
        // 패스트푸드
        '맥도날드', '버거킹', '맘스터치', '롯데리아', 'KFC', '도미노', '피자헛', '파파존스', '치킨', '피자',
        // 편의점
        'GS25', '세븐일레븐', 'CU', '씨유', '이마트24', '미니스톱', '편의점',
        // 배달앱
        '배달의민족', '요기요', '쿠팡이츠', '배달', '포장', '테이크아웃'
      ],
      '교통비': [
        '택시', '지하철', '버스', '교통', '주차', '주유', '기름', '카카오모빌리티', '우버',
        '티머니', '교통카드', '고속버스', '전국고속버스', '이동의즐거움', '카카오T'
      ],
      '쇼핑': [
        '쿠팡', '옥션', '지마켓', '11번가', '쇼핑', '구매', '아마존', '네이버쇼핑',
        '올리브영', '미리디', '자라쿠', '백화점', '마트', '이마트', '롯데마트', '홈플러스'
      ],
      '의료/건강': [
        '병원', '약국', '의료', '건강', '약', '치과', '안과', '한의원', '보건소'
      ],
      '교육': [
        '학원', '교육', '강의', '책', '도서', '온라인강의', '과외', '입시', '학습'
        // 주의: '대학교'는 제외 - 식당이 있으면 식비로 분류됨
      ],
      '문화/여가': [
        '영화', '콘서트', '공연', '문화', '넷플릭스', '디즈니', '게임',
        '노래방', '보라보라', '코인노래연습장', 'PC방', '아이센스리그PC'
      ],
      '주거/통신': [
    ...
    // 우선순위: 식비 > 교통비 > 쇼핑 > 의료/건강 > 교육 > 문화/여가 > 주거/통신
    // 식비 키워드가 가장 많으므로 먼저 확인
    const priorityOrder = ['식비', '교통비', '쇼핑', '의료/건강', '교육', '문화/여가', '주거/통신'];
    
    for (const categoryName of priorityOrder) {
      const keywords = rules[categoryName];
      const category = categories.find(cat => cat.name === categoryName);
      if (category && keywords.some(keyword => descLower.includes(keyword.toLowerCase()))) {
        return category._id;
      }
    }
  }
